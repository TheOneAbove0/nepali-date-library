...
export const NepaliDatePicker = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    return () => h('div', { class: 'nepali-date-picker' }, [
      h('div', { style: 'display:flex;gap:8px;align-items:center;' }, [
        h('button', { type: 'button', onClick: picker.goToPreviousMonth }, '<'),
        h('strong', `${MONTH_NAMES[picker.state.value.viewMonth - 1]} ${picker.state.value.viewYear}`),
        h('button', { type: 'button', onClick: picker.goToNextMonth }, '>'),
      ]),
      h('table', [
        h('tbody', picker.grid.value.weeks.map((week, weekIndex) => h('tr', { key: `w-${weekIndex}` },
          week.map((cell) => h('td', { key: `${cell.date.year}-${cell.date.month}-${cell.date.day}` }, [
            h('button', {
              type: 'button',
