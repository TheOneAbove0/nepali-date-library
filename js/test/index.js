...
describe('nepali-date-library', function() {
  ...
  describe('#toBik()', function() {
    ...
    it('should accept Date input', function() {
      assert.deepEqual(bs.toBik(new Date(Date.UTC(1950, 3, 13))), { day: 1, month: 1, year: 2007 });
    });
  });

  describe('#parseBsDate()', function() {
    it('should parse valid BS strings', function() {
      assert.deepEqual(bs.parseBsDate('2083-01-02'), { year: 2083, month: 1, day: 2 });
    });
  ...
  describe('#parseAdDate()', function() {
    it('should parse valid AD strings', function() {
      assert.deepEqual(bs.parseAdDate('2024-07-24'), { year: 2024, month: 7, day: 24 });
    });

    it('should parse Date input', function() {
      assert.deepEqual(bs.parseAdDate(new Date(Date.UTC(2024, 6, 24))), { year: 2024, month: 7, day: 24 });
    });
  ...
  describe('#manipulation()', function() {
    it('should add and subtract BS days, months, and years', function() {
      assert.deepEqual(bs.addBsDays('2083-01-30', 2), { year: 2083, month: 2, day: 1 });
      assert.deepEqual(bs.subtractBsDays('2083-02-01', 2), { year: 2083, month: 1, day: 30 });
      assert.deepEqual(bs.addBsMonths('2083-01-31', 1), { year: 2083, month: 2, day: 31 });
      assert.deepEqual(bs.subtractBsMonths('2083-02-31', 1), { year: 2083, month: 1, day: 31 });
      assert.deepEqual(bs.addBsYears('2083-01-31', 1), { year: 2084, month: 1, day: 31 });
      assert.deepEqual(bs.subtractBsYears('2084-01-31', 1), { year: 2083, month: 1, day: 31 });
    });
  });
  ...
  describe('#toBik_euro()', function() {
    Object.entries(test_data_for('toBik_euro')).forEach(function(_entry) {
      var gregorian = _entry[0];
      var expectedBikram = _entry[1];
  ...
  describe('#toGreg()', function() {
    ...
    test_data_for('toGreg').forEach(function(data) {
